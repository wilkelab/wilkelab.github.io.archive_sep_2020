...
*Now, instead of gathering data from all columns, gather only the data from columns `below10k`, `from20to30k`, and `from50to75k`.*
```{r}
# R code goes here.
```
...
*For the data set `diamonds` from the ggplot2 package, calculate the mean price 
for each combination of cut and clarity. Then use `spread()` to rearrange the table into a wide format, such that there is a column of mean prices for each cut level (Fair, Good, Very Good, etc.).*
```{r}
# R code goes here.
```
