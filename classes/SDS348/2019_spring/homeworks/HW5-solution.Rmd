...
```{r}
NCbirths %>% select(-Marital, -Smoke, -Plural) %>% scale() %>% prcomp() -> pca

pca_data <- data.frame(pca$x, NCbirths)

ggplot(pca_data, aes(x = PC1, y = PC2, color = factor(Marital))) + geom_point() + scale_color_colorblind()
ggplot(pca_data, aes(x = PC1, y = PC2, color = factor(Smoke))) + geom_point() + scale_color_colorblind()
ggplot(pca_data, aes(x = PC1, y = PC2, color = factor(Plural))) + geom_point() + scale_color_colorblind()
```
The samples do not seem to cluster based on the marital status and smoking habits of mothers. There seems to be one distinct cluster that mostly contains data for single births, and two clusers that contain data for single births, twins, and triplets.
