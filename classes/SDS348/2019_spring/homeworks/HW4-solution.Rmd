...
survived <- 
  ...
  mutate(survival_status = "survived")

deceased <- 
  deceased %>% 
  gather(sex, count, male:female) %>% 
  mutate(survival_status = "deceased")

titanic <- bind_rows(survived, deceased)
...
new_feed_names <- chickwts %>% left_join(feed_names)
...
*Answer approach: I could plot distributions of air time for the three airports given in the dataset. I could also run an ANOVA to assess whether the mean air times are different between the three airports.*
