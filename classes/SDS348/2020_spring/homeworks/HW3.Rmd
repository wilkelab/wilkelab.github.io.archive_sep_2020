...
For premature births, what are the **minimum** completed weeks of gestation and the **mean** birth weight for single babies, twins and triplets? Using the computed results, answer the question in 1-2 sentences. **HINT:** Use the function `min()` to determine the minimum completed weeks of gestation.
...
NCbirths %>%
  filter(Premie == 1) %>%
  group_by(Plural) %>%
  summarize(max_Weeks = min(Weeks), mean_BirthWeight = mean(BirthWeightGm))
